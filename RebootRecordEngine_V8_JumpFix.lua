-- RebootRecordEngine V7 - Stable Playback (no notifications)
--[[
...
local function exportRoute()
    ...
    if type(writefile) == "function" then
        ...
        for _, path in ipairs(paths) do
            ...
            if ok then
                StatusLabel:Set("Status: Saved → " .. path)
                return
            end
        end
        -- writefile exists but every path failed; fall through to clipboard.
    end

    -- ── Method 2: clipboard (setclipboard / toclipboard) ────
    local clipFn = type(setclipboard) == "function" and setclipboard
               or  type(toclipboard)  == "function" and toclipboard
               or  nil

    if clipFn then
        local ok = pcall(clipFn, json)
        if ok then
            StatusLabel:Set("Status: Copied to clipboard → paste as " .. filename)
            return
        end
    end

    -- ── Nothing worked ───────────────────────────────────────
    StatusLabel:Set("Status: Export failed - no write/clipboard method found")
end
...
local function playAnimationState(state, speed)
    ...
    track.Priority = (wanted == "jump" or wanted == "land")
