...
-- Gen2 loads Lucide icons through the executor asset API. Do not install a fake
-- getcustomasset() that returns an empty string, because Gen2 interprets that as
-- a real asset and the icons become permanently blank.
...
local __IconByName = {
    -- Common Reboot controls
    ["autoplay"] = "play", ["autoplay source"] = "shuffle", ["challenges"] = "trophy",
    ["tools"] = "wrench", ["visuals"] = "eye", ["utilities"] = "settings-2", ["settings"] = "settings",
    ["characters"] = "users", ["universal"] = "globe", ["main"] = "home", ["practice"] = "school",
    ["info"] = "info", ["font"] = "type", ["ui theme"] = "palette", ["static color"] = "palette",
    ["rainbow effect"] = "rainbow", ["vip name tag"] = "badge-check", ["custom name"] = "text-cursor-input",
    ["come back as male or female"] = "refresh-cw", ["kill all"] = "skull", ["spectate"] = "eye",
    ["fling"] = "wind", ["hat drop"] = "circle", ["chat spam"] = "message-square", ["crash server"] = "zap",
    ["credits"] = "heart",
}
...
local function __normalizeName(cfg)
    ...
    if not cfg.icon and cfg.name then
        local key = string.lower(tostring(cfg.name))
        cfg.icon = __IconByName[key]
        if not cfg.icon then
            if key:find("autoplay") then cfg.icon = "play"
            elseif key:find("settings") then cfg.icon = "settings"
            elseif key:find("visual") then cfg.icon = "eye"
            elseif key:find("tool") then cfg.icon = "wrench"
            elseif key:find("challenge") then cfg.icon = "trophy"
            elseif key:find("font") then cfg.icon = "type"
            elseif key:find("theme") then cfg.icon = "palette"
            elseif key:find("spectat") then cfg.icon = "eye"
            elseif key:find("sound") or key:find("chat") then cfg.icon = "volume-2"
            else cfg.icon = "circle" end
        end
    end
...
local function __makeTabAdapter(rawTab)
    ...
    function tab:CreateSection(title)
        ...
        local icon = type(title) == "table" and (title.icon or title.Icon) or nil
        return rawTab:CreateSection({name=name, icon=icon})
    end
    ...
    function tab:CreateLabel(text, icon, color, ignoreTheme)
        -- Gen2 SectionProps support an icon, so legacy labels retain their Lucide icon.
        local resolvedIcon = icon
        if not resolvedIcon then
            local key = string.lower(tostring(text or ""))
            resolvedIcon = __IconByName[key]
        end
        return rawTab:CreateSection({name=tostring(text or ""), icon=resolvedIcon})
    end
...
function Reboot:CreateWindow(cfg)
    ...
    local raw = __Gen2:CreateWindow({
        name = cfg.Name or cfg.name or "Reboot",
        subtitle = cfg.LoadingSubtitle or cfg.subtitle or ("Total Roblox Drama • " .. tostring(cfg.LoadingTitle or "Reboot")),
        theme = theme == "Default" and "default" or theme,
        icon = cfg.Icon or cfg.icon or "rocket",
        showName = cfg.ShowName or cfg.showName or "Reboot",
        showIcon = cfg.ShowIcon or cfg.showIcon or "rocket",
    })
    local window = {
        _raw = raw,
        Flags = __RebootFlags,
...
pcall(function()
    ...
    SettingsTab:CreateLabel("Creds to Ian for Autoplay <3", "heart")
    SettingsTab:CreateLabel("Creds to dw Team for Bp Autoplay <3", "heart")
end)
...
-- TrdTAS manifest: https://raw.githubusercontent.com/manningian744-droid/Open-Source-Autoplay/main/manifest.txt
local TAS_RouteSourceMode = "TrdTAS Autoplays"
local TAS_RouteSources = TRDTAS_RouteSources

local function TAS_SetRouteSourceMode(mode)
    if mode == "Dramaware" then
        TAS_RouteSourceMode = "Dramaware"
        TAS_RouteSources = DRAMAWARE_RouteSources
    else
        TAS_RouteSourceMode = "TrdTAS Autoplays"
        TAS_RouteSources = TRDTAS_RouteSources
    end
end
...
AutoplayTab:CreateDropdown({
    Name = "Autoplay Source",
    Icon = "shuffle",
    Options = {"Dramaware", "TrdTAS Autoplays"},
    CurrentOption = {"TrdTAS Autoplays"},
    MultipleOptions = false,
    Flag = "AutoplaySource",
    Callback = function(Option)
        local selected = (type(Option) == "table") and (Option.Option or Option[1]) or Option
        TAS_SetRouteSourceMode(tostring(selected or "TrdTAS Autoplays"))
        TAS_RouteCache = {}
        TAS_RouteCacheDirect = {}
        task.spawn(TAS_PrefetchAllRoutes)
        Rayfield:Notify({
            Title = "Autoplay Source",
            Content = "Switched to " .. TAS_RouteSourceMode,
            Duration = 3,
            Image = "shuffle",
        })
    end,
