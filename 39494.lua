...
local function ExportRouteToFile(routeData)
    if type(writefile) ~= "function" then
        ...
        if type(setclipboard) == "function" then
            copied = pcall(setclipboard, HttpService:JSONEncode(routeData))
        elseif type(toclipboard) == "function" then
            copied = pcall(toclipboard, HttpService:JSONEncode(routeData))
        end

        Rayfield:Notify({
            Title = "JSON Export",
            Content = copied
                and "No file API found. JSON copied to clipboard instead."
                or "This executor has no file-writing API.",
            Duration = 3
        })
        return false
    end
