--[[
    Celestia Hub
    Total Roblox Drama script project

    Cleaned/rebranded build based on the supplied source.
    External route loader: none.
]]--
...
local _CelestiaHubHttp = game:GetService("HttpService")
local _CelestiaHub_Elements = {}
...
local function CelestiaHub_EnsureFolders()
    pcall(function()
        if not isfolder("CelestiaHub") then makefolder("CelestiaHub") end
        if not isfolder("CelestiaHub/Configs") then makefolder("CelestiaHub/Configs") end
    end)
end

local function CelestiaHub_ListConfigs()
    local names = {}
    pcall(function()
        CelestiaHub_EnsureFolders()
        local files = (listfiles and listfiles("CelestiaHub/Configs")) or {}
...
local _CelestiaHub_SkipFlags = { CelestiaHubConfigDropdown = true }

local function CelestiaHub_PackColor(c)
    return {R = math.floor(c.R * 255 + 0.5), G = math.floor(c.G * 255 + 0.5), B = math.floor(c.B * 255 + 0.5)}
end

local function CelestiaHub_UnpackColor(t)
    return Color3.fromRGB(t.R or 0, t.G or 0, t.B or 0)
end

local function CelestiaHub_SaveConfig(name)
    CelestiaHub_EnsureFolders()
    local data = {}
    for flag, obj in pairs(Rayfield.Flags) do
        if not _CelestiaHub_SkipFlags[flag] then
            pcall(function()
                if obj.Type == "ColorPicker" then
                    if obj.Color then
                        data[flag] = CelestiaHub_PackColor(obj.Color)
                    end
    ...
    local ok, json = pcall(function() return _CelestiaHubHttp:JSONEncode(data) end)
    if ok then
        pcall(function() writefile("CelestiaHub/Configs/" .. name .. ".json", json) end)
    end
end

local function CelestiaHub_LoadConfig(name)
    local path = "CelestiaHub/Configs/" .. name .. ".json"
    local exists = false
    pcall(function() exists = isfile(path) end)
    if not exists then return false end
    local ok, data = pcall(function()
        return _CelestiaHubHttp:JSONDecode(readfile(path))
    end)
    if ok and data then
                ...
                task.spawn(function()
                    pcall(function()
                        ...
                        if flagObj and flagObj.Type == "ColorPicker" and type(v) == "table" then
                            v = CelestiaHub_UnpackColor(v)
                        end
                        local el = _CelestiaHub_Elements[f]
...
local function CelestiaHub_PatchTab(tab)
        ...
        if orig then
            tab[m] = function(s, cfg, ...)
                ...
                if cfg and type(cfg) == "table" and cfg.Flag and el then
                    _CelestiaHub_Elements[cfg.Flag] = el
                end
                return el
            end
        end
...
local function saveTheme(theme)
    pcall(function()
        if not isfolder("CelestiaHub") then makefolder("CelestiaHub") end
        writefile("CelestiaHub/Theme.txt", theme)
    end)
end
...
local function loadTheme()
    local theme = nil
    pcall(function()
        if isfile("CelestiaHub/Theme.txt") then
            local t = readfile("CelestiaHub/Theme.txt"):match("^%s*(.-)%s*$")
...
do
    ...
    _G._CelestiaHub_SetLedge = function(v)
        _LedgeEnabled    = v
        _FpsBounceEnabled = v
    end
    ...
    _G._CelestiaHub_LHJGui = _LHJGui
...
if isLobby then
    local Window = Rayfield:CreateWindow({
        Name = "Celestia Hub",
        LoadingTitle = "Celestia Hub (" .. game_type .. ")",
        LoadingSubtitle = "Total Roblox Drama",
        ShowText = "Celestia Hub",
        Theme = chosen_theme,
        DisableRayfieldPrompts = true,
        DisableBuildWarnings = true,
        CustomUIStrings = {
            ["Rayfield"] = "Celestia Hub",
            ["Hide"] = "Hide",
            ["Show"] = "Show"
        },
        ConfigurationSaving = {
            Enabled = true,
            FolderName = "CelestiaHub",
            FileName = "Config"
        }
    })
    local InfoTab = Window:CreateTab("Info", "info")
    InfoTab:CreateSection("Celestia Hub")
...
local function equipGear(toolName)
    ...
    if not _gearFolder then warn("[CelestiaHub] Gear folder not found!") return end
    local _gearTool = _gearFolder:FindFirstChild(toolName)
    if not _gearTool then warn("[CelestiaHub] Tool not found: " .. toolName) return end
...
SettingsTab:CreateButton({
    ...
    Callback = function()
        ...
        local path = "CelestiaHub/Configs/" .. _dwSelectedConfig .. ".json"
        local deleted = false
        pcall(function()
...
do
    ...
    local function _csb_getSavedSkins()
        ...
        pcall(function()
            if not isfolder("CelestiaHub") then makefolder("CelestiaHub") end
            if not isfolder("CelestiaHub/CustomSkins") then makefolder("CelestiaHub/CustomSkins") end
            local files = listfiles and listfiles("CelestiaHub/CustomSkins") or {}
            for _, path in ipairs(files) do
                local name = tostring(path):match("([^/\]+)%.json$")
                if name and name ~= "" then table.insert(names, name) end
            end
    ...
    CharactersTab:CreateInput({
        ...
        Callback = function(text)
            ...
            pcall(function()
                if not isfolder("CelestiaHub") then makefolder("CelestiaHub") end
                if not isfolder("CelestiaHub/CustomSkins") then makefolder("CelestiaHub/CustomSkins") end
                writefile("CelestiaHub/CustomSkins/" .. saveName .. ".json", game:GetService("HttpService"):JSONEncode(data))
            end)
    ...
    _csb_loadDropdown = CharactersTab:CreateDropdown({
        ...
        Callback = function(Option)
            ...
            local path = "CelestiaHub/CustomSkins/" .. name .. ".json"
    ...
    CharactersTab:CreateButton({
        ...
        Callback = function()
            ...
            local path = "CelestiaHub/CustomSkins/" .. _csb_selectedLoad .. ".json"
            local deleted = false
            pcall(function()
                if isfile(path) then
                    delfile(path)
...
local function _rec_getFiles()
    ...
    pcall(function()
        if not isfolder("CelestiaHub") then makefolder("CelestiaHub") end
        if not isfolder("CelestiaHub/Recordings") then makefolder("CelestiaHub/Recordings") end
        local files = (listfiles and listfiles("CelestiaHub/Recordings")) or {}
        for _, path in ipairs(files) do
...
AutoplayTab:CreateInput({
    ...
    Callback = function(text)
        ...
        pcall(function()
            ...
            local filePath = "CelestiaHub/Recordings/" .. name .. ".lua"
                    ...
                    if Hum and Hum.WalkSpeed == 0 then
                        ...
                        if not _rec_autoRoundPlay or not TAS_IsInChallenge() then
                            ...
                            if _rec_selectedLoad and _rec_selectedLoad ~= "" and _rec_selectedLoad ~= "(no recordings)" and _rec_selectedLoad ~= "(nothing)" then
                                local filePath = "CelestiaHub/Recordings/" .. _rec_selectedLoad .. ".lua"
