...
local function playRecordedRoute()
    ...
    task.spawn(function()
        ...
        for index, sample in ipairs(route) do
            if not playing then
                break
            end
            ...
            local nextSample = route[index + 1]

            if root.Parent and type(p) == "table" and #p >= 3 then
                ...
                -- Make the Humanoid/Animate system visibly move while
                -- the recorded root position is being replayed.
                if nextSample and type(nextSample.position) == "table" then
                    local a = Vector3.new(p[1], p[2], p[3])
                    local b = Vector3.new(
                        nextSample.position[1],
                        nextSample.position[2],
                        nextSample.position[3]
                    )

                    local delta = b - a
                    local horizontal = Vector3.new(delta.X, 0, delta.Z)

                    if horizontal.Magnitude > 0.02 then
            ...
            -- Drive the visual animation from the recorded Humanoid state.
            ...
            elseif state == "Running" or state == "Walking" then
