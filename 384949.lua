--[[
    Rayfield Gen1 Practice Route Recorder
...
-- Full JSON export: writes the complete route to a file instead of the clipboard.
local function ExportRouteToFile(route)
    ...
    local HttpService = game:GetService("HttpService")
    local json = HttpService:JSONEncode(route)
    local stamp = os.date("%Y%m%d_%H%M%S")
    local safeName = tostring(route.name or "RebootRoute"):gsub("[^%w_%-]", "_")
    local filename = safeName .. "_" .. stamp .. ".json"
...
-- Preview the currently recorded route locally.
...
local function PlayRecordedRoute()
    if playPreviewing then
        Rayfield:Notify({
            Title = "Play Record",
            Content = "A preview is already playing.",
            Duration = 3
        })
        return
    end

    if type(route) ~= "table" or type(route.samples) ~= "table" or #route.samples == 0 then
        Rayfield:Notify({
            Title = "Play Record",
    ...
    if not root then
        Rayfield:Notify({
            Title = "Play Record",
            Content = "Character/HumanoidRootPart not found.",
            Duration = 4
        })
        return
    end
...
local function exportRoute()
    ...
    local filename = routeName:gsub("[^%w%-_]", "_") .. ".json"
    ...
    elseif typeof(toclipboard) == "function" then
        local ok = pcall(toclipboard, json)
        copied = ok
    end
    ...
    if copied then
        Rayfield:Notify({
            Title = "JSON Exported",
            Content = "Copied " .. filename .. " to clipboard.",
            Duration = 5,
        ...
        Rayfield:Notify({
            Title = "JSON Ready",
            Content = "Clipboard API unavailable. JSON length: " .. tostring(#json),
            Duration = 5,
            Image = "file-json"
        })
        print("===== " .. filename .. " =====")
